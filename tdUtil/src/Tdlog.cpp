...
char* Tdlog::get_time_str(bool is_write)
{
    time_t now = {0};
    ...
    if (is_write)
    {//用来写日志
        sprintf(m_time_str, "%04d-%02d-%02d %02d:%02d:%02d.%03ld",
...
int Tdlog::writeline(uint level, const char* format_str, ...)
{
    ...
    if(m_fp!=NULL)
    {
        ...
        write_len = fprintf(m_fp, "[%s][%s]", get_time_str(true), s_level_str[level-1]);
        va_list p_list;
        ...
        int write_len = fprintf(stdout, "[%s] [%s] ", get_time_str(true), s_level_str[level-1]);
        va_list p_list;
        va_start(p_list, format_str);
        write_len += vfprintf(stdout, format_str, p_list);
        va_end(p_list);
        write_len += fprintf(stdout, "\n");
    }
    m_lock.unlock();
    return write_len;
}

void Tdlog::clean_logs()
{
    if (!m_is_cleaning)
    {
        m_is_cleaning = true;
...
#ifdef WIN32
void Tdlog::clean_logs_thread_fun(void* param)
#else
void* Tdlog::clean_logs_thread_fun(void* param)
#endif
{
    Tdlog* log_ptr = (Tdlog*)param;
...
#ifdef WIN32
bool Tdlog::get_log_files(const string& log_path, file_list& files)
{
...
string Tdlog::get_log_dir(const string& log_path)
{
...
log_inst    TdlogFactory::m_inst_list[MAX_LOG_INSTANCE] = {0};
uint        TdlogFactory::m_inst_num = 0;
MutexLock   TdlogFactory::m_lock;

TdlogFactory::TdlogFactory(void)
{
}
...
Tdlog* TdlogFactory::get_instance(const char* name)
{
    Tdlog* plog = NULL;
...
void TdlogFactory::free_instance(const char* name)
{
    m_lock.lock();
    for (int i = 0; i < m_inst_num; i++)
    {
...
void TdlogFactory::free_all_inst()
{
    m_lock.lock();
    for (int i = 0; i < m_inst_num; i++)
    {
