...
#define SET_LOG_NAME(filepath)              TdlogFactory::get_instance(DEFAULT_MODULE)->set_log_filepath(filepath)
#define SET_LOG_LEVEL(level)                TdlogFactory::get_instance(DEFAULT_MODULE)->set_log_level(level)
#define SET_LOG_SIZE(size)                  TdlogFactory::get_instance(DEFAULT_MODULE)->set_log_size(size)
#define SET_LOG_SPACE(size)                 TdlogFactory::get_instance(DEFAULT_MODULE)->set_log_space(size)

#define LOG_TRACE(format, ...)              TdlogFactory::get_instance(DEFAULT_MODULE)->writeline(LOG_LEVEL_TRACE, format, ## __VA_ARGS__)
#define LOG_INFO(format, ...)               TdlogFactory::get_instance(DEFAULT_MODULE)->writeline(LOG_LEVEL_INFO, format, ## __VA_ARGS__)
#define LOG_WARNING(format, ...)            TdlogFactory::get_instance(DEFAULT_MODULE)->writeline(LOG_LEVEL_WARNING, format, ## __VA_ARGS__)
#define LOG_ERROR(format, ...)              TdlogFactory::get_instance(DEFAULT_MODULE)->writeline(LOG_LEVEL_ERROR, format, ## __VA_ARGS__)

#else

//多实例API
#define SET_LOG_NAME(module, filepath)      TdlogFactory::get_instance(module)->set_log_filepath(filepath)
#define SET_LOG_LEVEL(module, level)        TdlogFactory::get_instance(module)->set_log_level(level)
#define SET_LOG_SIZE(module, size)          TdlogFactory::get_instance(module)->set_log_size(size)
#define SET_LOG_SPACE(module, size)         TdlogFactory::get_instance(module)->set_log_space(size)

#define LOG_TRACE(module, format, ...)      TdlogFactory::get_instance(module)->writeline(LOG_LEVEL_TRACE, format, ## __VA_ARGS__)
#define LOG_INFO(module, format, ...)       TdlogFactory::get_instance(module)->writeline(LOG_LEVEL_INFO, format, ## __VA_ARGS__)
#define LOG_WARNING(module, format, ...)    TdlogFactory::get_instance(module)->writeline(LOG_LEVEL_WARNING, format, ## __VA_ARGS__)
#define LOG_ERROR(module, format, ...)      TdlogFactory::get_instance(module)->writeline(LOG_LEVEL_ERROR, format, ## __VA_ARGS__)
...
public:
    TdlogFactory(void);
    virtual ~TdlogFactory(void);

    static Tdlog* get_instance(const char* name);
